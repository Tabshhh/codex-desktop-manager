...
- 列出已保存账号，并支持一键切换
- 兼容旧版本本地快照，并对损坏快照做容错加载
- 切换前自动备份当前 live 状态
- 如果切换验证失败，可以回滚上一次切换
- 基于本地 Codex 会话数据展示当前账号状态
- 使用快照中保存的账号认证信息刷新该账号额度
- 支持删除本地保存的账号快照
- 账号池支持按实际可用额度排序，并将当前账号固定置顶
- 记住桌面窗口大小和位置

## 使用流程

1. 先在官方 Codex Desktop 中登录一个你要保存的账号，并确认它已经可以正常打开和使用。
2. 打开 `Codex Desktop Manager`，点击保存当前账号，把这个 live 登录态保存成一个本地快照。
3. 如果你有更多账号，回到 Codex Desktop 切换登录下一个账号，然后重复保存，逐个建立账号池。
4. 后续需要换号时，直接在账号池中点击 `Switch`，工具会先备份当前 live 状态，再写入目标快照并重启 Codex。
5. 如果切换后校验失败或桌面端状态异常，可以使用 `Rollback last switch` 恢复到上一次备份。
6. 需要重新查看某个已保存账号的额度时，使用该账号卡片上的刷新动作重新读取对应认证状态。
